...
export default class CannonUtils {
  static createTrimesh(geometry, scale) {
    let vertices;
    ...
    for (let i = 0; i < vertices.length; i ++) {
      newVertices.push(vertices[i] * scale)
    }
    const indices = Object.keys(newVertices).map(Number);
    return new CANNON.Trimesh(newVertices, indices);
  }
  ...
  offsetCenterOfMass(body, centreOfMass) {
